...
void radicon_joint1(bool _up, bool _down){
  int _angle = analogRead(PIN_JOINT_1_L_POT);
  _angle     = mapf(_angle, POT_READ_MIN_1_L, POT_READ_MAX_1_L, POT_ANGLE_MIN_1_L, POT_ANGLE_MAX_1_L);
...
void radicon_run(int A, int B, int C, int D, int G, int F){
  if(A < 1200){
    front_l = true;
    back_l = false;
  }
...
void radicon(int MR8_A, int MR8_B, int MR8_C, int MR8_D, int MR8_G, int MR8_F){
  /*
  Serial.print("MR8_A = ");
  Serial.print(MR8_A);
  Serial.print("  MR8_B = ");
  ...
  */
  ...
  radicon_run(MR8_A, MR8_B, MR8_C, MR8_D, MR8_G, MR8_F);
}
